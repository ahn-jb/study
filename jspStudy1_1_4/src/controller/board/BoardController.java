...
//			System.out.println("bunryu"+bunryu);
			...
			int no = (Integer)session.getAttribute("cookNo");
			...
			MemberDTO dto2 = dao2.getSelectOneNo(cookNo);
			request.setAttribute("menu_gubun", "board_view");
			request.setAttribute("dto", dto);			
			request.setAttribute("Id", dto2.getId());	
			request.setAttribute("Grade", dto2.getGrade());
			request.setAttribute("bunryu",bunryu);
			request.setAttribute("search",search);
