...
<%
	...
	if(count > 0 ){
		list = dao_1.getList2(startRow, endRow);
	}
	
	String check = request.getParameter("check");
	if(check == null){
		if(count - currentPage*(pageSize-1) < 0){//완전한 식은 아니다. 
			currentPage = count/pageSize + (count%pageSize == 0 ? 0:1) ;
			pageNum = String.valueOf(currentPage);
			
			out.println("<script>");
			out.println("location.href='../board/list.jsp?pageSize="+pageSize+"&pageNum="+pageNum+"&check=1';");
			out.println("</script>");
		}
	}
%>
...
// 							int number = count - (currentPage-1)*pageSize;
							...
							for(int i=0; i<list.size(); i++){ 
								...
								int number = dto1.getP_no_result();
...
// 								System.out.println(number);
							...
							<tr>
								...
								<a href="#" onclick="view('<%=dto1.getNo()%>');" ><%=dto1.getSubject() %>[<%=answer_boolean %>]</a>
								</td>
						...
						<tr>
							<td colspan="9" align="leff">
								...
								%>
									<a href="list.jsp?pageNum=<%=1%>&pageSize=<%=pageSize%>">[첫페이지]</a>
								<%
...
<!-- 									[다음] -->
								...
								%>
									<a href="list.jsp?pageNum=<%=pageCount%>&pageSize=<%=pageSize%>">[끝페이지]</a>
								<% 
								}
								%>
							</td>
