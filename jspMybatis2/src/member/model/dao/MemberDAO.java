...
//	public List<MemberDTO> getSelectAll(){//가입된 모든 회원정보 
...
//	}
	...
	public MemberDTO getSelectOneNo(int no) {//no으로 찾는  회원 정보
		...
		SqlSession session = MybatisManager.getInstance().openSession();
		MemberDTO dto = session.selectOne("member.getSelectOneNo",no);
		session.close();
//		try {
...
//		}
	...
	public int getSelectId(String id) {
		...
		SqlSession session = MybatisManager.getInstance().openSession();
		int result= session.insert("member.getSelectId",id);
		session.commit();
		session.close();
//		int result =0;
//		try {
...
//	}
	...
	public MemberDTO Login(MemberDTO dto) {//로그인 		
		...
		map.put("dto", dto);
		SqlSession session = MybatisManager.getInstance().openSession();
		MemberDTO dto2 = session.selectOne("member.Login",map);
		session.close();
...
//		}
	...
	public void loginCounterfail(MemberDTO dto) {//로그인 실패 카운트
		...
		map.put("dto", dto);
		SqlSession session = MybatisManager.getInstance().openSession();
		int result= session.insert("member.loginCounterfail",map);
		session.commit();
		session.close();
//		try {
...
//		}
	...
	public void loginCounterSucsess(MemberDTO dto) {//로그인 성공시 카운트 리셋
		...
		map.put("dto", dto);
		SqlSession session = MybatisManager.getInstance().openSession();
		int result= session.insert("member.loginCounterSucsess",map);
		session.commit();
		session.close();
//		try {
...
//		asd
		...
		map.put("dto", dto);
		SqlSession session = MybatisManager.getInstance().openSession();
		int result= session.insert("member.update",map);
		session.commit();
		session.close();
...
//		qwe
		SqlSession session = MybatisManager.getInstance().openSession();
		int result= session.insert("member.delete",no);
		session.commit();
		session.close();
//		int result = 0;
//		try {
