...
<!-- 					 onkeypress="nonHangulSpecialKey()" -->
						...
						<input type="text" id="asdqwe" name="asdqwe" value="">
					</td>
				</tr>
				<tr>
					<td >비밀번호</td>
				...
				<tr>
					<td >성별</td>
					<td>
						<input type="radio"   name="gender" value="M" onclick="genderChk('M');">남자
						<input type="radio"  name="gender" value="F" onclick="genderChk('F');">여자<br>
						<input type="hidden"  name="gender" id="gender" value="">
					</td>
				</tr>
				<tr>
					<td >생년</td>
					<td><input type="text" id="bornYear" name="bornYear"></td>
				</tr>
				<tr>
					<td>주소:</td>
					<td>
...
<script type="text/javascript">
	function id_check_win_open(){
		window.open("${path}/member/id_check_win_open.do","아이디확인","width=640, height=480, toolbar=no, menubar=no, scrollbars=no, resizable=yes");
		
	}
	function genderChk(value1){
		$("#gender").val(value1);
	}
	function id_check(){
		var id = $("#id").val();
	...
	function save(){
		if(document.joinForm.id.value.trim() ==""){
			...
			return false;
		}else if(document.joinForm.id.value.trim() != document.joinForm.asdqwe.value.trim()){
			alert('아이디를 확인해주세요.')
			document.joinForm.id.focus();
			return false;
